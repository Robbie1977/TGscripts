...
for fname in filelist:
    ...
    if (hostn + '\n') in stoplist:
        ...
        try:
            if os.path.exists(fname):
                ...
                with open("Warp.log", "a") as myfile: # Log entry for process time and error checking
                    myfile.write(basename + ', Started BG affine trans, ' + procid + ', ' + str(datetime.datetime.now()) + '\n')
		...
		Axoutput = outdir + basename + '-affine.nrrd'
...
#                        print 'warp (5) returned: %d' % return_code
                ...
                if os.path.exists(Axoutput):
                    ...
                    return_code = subprocess.call('nice ' + cmtkdir + 'reformatx -o %s --floating %s %s %s' % (Axoutput, FloatFile, Tfile, Axform), shell=True)
                    print 'reformatx returned: %d' % return_code
                print 'Completed affine transformation for %s.' % basename
                if os.path.exists(Axoutput):    
                    ...
                    with open("FLbrainA.txt", "a") as myfile:
                        myfile.write(Axoutput + '\n')
                    ...
                    with open("Warp.log", "a") as myfile: # Log entry for process time and error checking
                        myfile.write(basename + ', Finished BG affine trans, ' + procid + ', ' + str(datetime.datetime.now()) + '\n')

                else:
                    print 'Failed affine for %s.' % basename
                    os.rename(fname.replace('_blue',''),fname.replace('_blue','_blue'))
                    with open("Warp.log", "a") as myfile: # Log entry for process time and error checking
