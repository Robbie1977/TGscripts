...
while read host ; do
  if [ `tail -n 500 /tmp/130524/VFB/IMAGE_DATA/Janelia2012/TG/logs/PLwarp.log | grep "$host" | wc -l` -gt 0 ]; then
		exit_code=45
  else	
...
if [ `grep processor /proc/cpuinfo | wc -l` -ge 2 ] && \
  ! grep -q "Pentium" /proc/cpuinfo && [ `pgrep -u s1002628 python | wc -l` -lt 1 ] && [ "$(w -h | wc -l)" == 0 ]; then
	...
	if [ $host == 'zuniga' ]; then
		echo $(date), $RN, $NR, $ER, $OU, $NA
		echo $(date), $RN, $NR, $ER, $OU, $NA >> /tmp/130524/VFB/IMAGE_DATA/Janelia2012/TG/logs/Recruit.log
	fi
        #sleep 1.1s
