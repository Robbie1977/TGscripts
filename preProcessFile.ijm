...
function processfile(fromfname, tofile) {
  ...
  hmax = 4079;
    ...
    else
    {
     if (File.exists(fromfname))
     {
      	if (File.exists(tofile + "_no")) // FIX ME!!
      	{
            ...
            filelist = File.openAsString("FLpre.txt");
	...
	run("8-bit");
	wait(300);
	run("Nrrd ... ", "nrrd=[" + name + "_green.nrrd]");
	wait(800);
	close();
...
if (c>0) {
	...
	File.append(replace(rname,".nrrd","_blue.nrrd"), "FLwarp.txt") 
}else{
